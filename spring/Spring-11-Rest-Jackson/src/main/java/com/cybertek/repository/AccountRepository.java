...
//    List<Account> findAllByCountryEqualsOrState(String country, String state);
//
...
//    @Query(value = "select * from account_details where name ilike concat('%', ?1, '%') or country ilike concat('%', ?1, '%') or address ilike concat('%', ?1, '%')", nativeQuery = true)
//    List<Account> retrieveBySearchCriteria(String pattern);
//
////Write a native query to read all accounts with an age lower than a specific value
//    @Query(value = "select * from acoount_details where age>?1", nativeQuery = true)
